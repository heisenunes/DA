 ...
 * @param q queue that stores the vertices that makes part of the BFS
 * @param e outgoing/incoming edge where 'w' can be the destination or the origin
 * @param w vertex that is tested if is visited or not
 * @param residual flow that pass through the edge
 * @param maxDelivery map that updates and stores the max Delivery values related to the water reservoirs
 * @param Demand map that updates and stores the demand values related to the cities.
 * @param flag to distinguish if we are testing incoming or outgoing edge
 * 
 */

template <class T>
void testAndVisit(std::queue< Vertex<T>*> &q, Edge<T> *e, Vertex<T> *w, double residual, unordered_map<Vertex<T>*, double>* maxDelivery, unordered_map<Vertex<T>*, double>* Demand, int flag) {
  //unordered_map<string, Reservoir> reservoirs = getReservoirs("Project1DataSetSmall/Project1DataSetSmall/Reservoirs_Madeira.csv");
  //unordered_map<string, City> cities = getCities("Project1DataSetSmall/Project1DataSetSmall/Cities_Madeira.csv");
//  if(w->getInfo()[0] == 'C' && (*Demand)[w] == cities.find(w->getInfo())->second.getDemand() && flag == 1) {
  //}

  if(w->getInfo()[0] == 'T' && (*Demand)[e->getOrig()] == cities.find(e->getOrig()->getInfo())->second.getDemand()) {

  }

  else if(w->getInfo()[0] == 'R' && (*maxDelivery)[w] == reservoirs.find(w->getInfo())->second.get_Maximum_Delivery() && flag == 1) {
  }
 ...
 * 
 * @tparam T The datatype used for the vertices in the graph
 * @param g A pointer to the graph representing the flow network
 * @param s the supersource of the flow network
 * @param t the supersink of the flow network
 * @param maxDelivery map that updates and stores the max Delivery values related to the water reservoirs
 * @param Demand map that updates and stores the demand values related to the cities.
 * 
...
// Mark all vertices as not visited
  ...
  while( ! q.empty() && ! t->isVisited()) {
    ...
    for(auto e: v->getAdj()) {
      testAndVisit(q, e, e->getDest(), e->getWeight() - e->getFlow(), &(*maxDelivery), &(*Demand),1);
    }
    // Process incoming edges
    for(auto e: v->getIncoming()) {
      testAndVisit(q, e, e->getOrig(), e->getFlow(), &(*maxDelivery), &(*Demand), 0);
    }
  }
 ...
 * @param Demand map that updates and stores the demand values related to the cities.
 * 
 * @return flow of the minimum augmenting path
 * 
 */
template <class T>
 ...
 * @param s the supersource of the flow network
 * @param t the supersink of the flow network
 * @param f flow that will run the path.
 * 
 */
...
/**
 * @brief Determines the maximum flow in the network.
 * 
 * @tparam T The datatype used for the vertices in the graph
 * @param g A pointer to the graph representing the flow network
 * @param source the supersource of the flow network
 * @param target the target of the flow network
 * 
 * @return Maximum flow of target in a flow network
 */
template <class T>
double edmondsKarp(Graph<T> *g, std::string source, std::string target) {
  ...
  Vertex<T>* c = g->findVertex(target);
...
/**
 * @brief Print which cities are in deficit
 * 
 * @tparam T The datatype used for the vertices in the graph.
 * @param g A pointer to the graph representing the flow network
 */
 ...
 */
template <class T>
void dfs_target(Graph<T> *g, const T& current, const T& dest, std::vector<Edge<T>*>& path, std::vector<std::vector<Edge<T>*> >& paths) {
    Vertex<T>* source = g->findVertex(current);
    ...
    if(current == dest) {
        ...
        for(auto e : source->getAdj()) {
            ...
            if(!u->isVisited() && e->getWeight() - e->getFlow() > 0) {
                path.push_back(e);
                dfs_target(g,u->getInfo(), dest, path, paths);
                path.pop_back();
            }
        }
    }
 ...
 */
template <class T>
std::vector<std::vector<Edge<T>*> > getPaths(Graph<T> *g,const T& source, const T& dest) {
    ...
    dfs_target(g,source, dest, path, paths); //find a path between source and destination with dfs
...
void rebalanceFlow(Graph<T> *g){
  ...
  while (continueBalancing) {
    ...
    for (Edge<T>* edge : pipes) {
      ...
      auto paths = getPaths(g,edge->getOrig()->getInfo(), edge->getDest()->getInfo());
      if (paths.empty()) {
...
/**
 * @brief Print which cities are in deficit after removing a reservoir from the flow network
 * 
 * @tparam T The datatype used for the vertices in the graph.
 * @param g A pointer to the graph representing the flow network
 * @param reservoir 
...
/**
 * @brief Print which cities are in deficit after removing a pumping station from the flow network.
 * 
 * @tparam T The datatype used for the vertices in the graph.
 * @param g A pointer to the graph representing the flow network.
 * @param pump_station a string representing the pumping station.
 */
...
/**
 * @brief Print which cities are in deficit after removing a pipe(edge) from the flow network.
 * 
 * @tparam T The datatype used for the vertices in the graph.
 * @param g A pointer to the graph representing the flow network.
 * @param pipeId A integer representing the a pipe(edge) of the flow network.
 */

template<class T>
void t3_3(Graph<T> *g, int pipeId){
  ...
  auto it = pipes.find(pipeId);
    if (it != pipes.end()) {
        // Se encontrado, acessa o objeto Pipe através do iterador
        Pipe& edge = it->second;

        edge_source = edge.getService_point_a();
        edge_dest = edge.getService_point_b();
        edge_weight = edge.getCapacity();
     
     g->removeEdge(edge.getService_point_a(), edge.getService_point_b());
        
    } else {
        std::cout << "Pipe não encontrado!" << std::endl;
  ...
  for(auto v: g->getVertexSet()){
   ...
   g->addEdge(edge_source,edge_dest, edge_weight);
